...
# uuid:   f0b49efc-7320-11ed-9c2f-0f859daf6789
...
#NOTE: Typically these are targets used in actions and can be resolved by using the $PY...
...
Help("""
      Type: `scons` - Create a venv if it doesn't exist and install packages
            ...
            `scons jupyter` - Launch the Jupyter notebook environment
            `scons jupyter_extensions` - Install the Jupyter notebook extensions
      """)
...
env.Alias('venv', [
    '.venv',
    env.Alias('packages'),
    env.Alias('jupyter_extensions'), # This assumes that jupyter is installed and the extensions are installed...
])

env.Alias('remove', [
    env.Alias('remove_venv'),
    env.Alias('clean'),
])
...
#NOTE: This one should be automatically executed when jupyter is installed
